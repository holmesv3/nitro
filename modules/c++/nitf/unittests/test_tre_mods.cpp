...
#define TEST_STR_EQ(X, Y) CHECK_THAT(X, Catch::Matchers::Equals(Y));

TEST_CASE("testNestedMod")
{
    CHECK( nitf_PluginRegistry_PreloadedTREHandlerEnable("ACCHZB", NRT_TRUE) );
    ...
    CHECK(exists);
    TEST_STR_EQ(nitf_TRE_getField(tre, "UNIAAH[0]")->raw, "abc");
    TEST_STR_EQ(nitf_TRE_getField(tre, "UNIAAH[1]")->raw, "cba");
...
TEST_CASE("testClone")
{
    ...
    CHECK(clonedField != NULL);
    TEST_STR_EQ(clonedField->raw, "fyi");
...
TEST_CASE("testSize")
{
    CHECK( nitf_PluginRegistry_PreloadedTREHandlerEnable("AIMIDB", NRT_TRUE) );
    nitf_Error error;
    int treLength;
...
TEST_CASE("iterateUnfilled")
{
    ...
    {
        CHECK(nitf_TRECursor_iterate(&cursor, &error) != 0);
        ++numFields;
    }
